...
# ===================================================
    ...
    # ===================================================
    # CANDIDATOS EODHD PARA INDICADORES SIN MATCH/CONFIG
    # ===================================================

    df_auditoria = pd.DataFrame(auditoria_eodhd)

    faltantes_eodhd = df_auditoria[
        df_auditoria["Estado"].isin([
            "❌ SIN CONFIG",
            "❌ SIN MATCH",
        ])
    ].copy()

    if not faltantes_eodhd.empty:
        ...
        for indicador_faltante in faltantes_eodhd["Indicador"]:
        ...
        st.write(
            f"### CANDIDATOS EODHD — {currency}"
        )

        st.dataframe(
            pd.DataFrame(sugerencias_matching),
            use_container_width=True,
            hide_index=True,
        )
